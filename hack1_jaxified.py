...
# %%
from jax.numpy import corrcoef as jax_corrcoef
...
for layer in tqdm(range(11), leave=False):
    ...
    for i in range(2):
        ...
        for j in range(2):
            ...
            a_jaxified = jnp.array(a.numpy())
            b_jaxified = jnp.array(b.numpy())

            print (type(a), type(b))
            corr = np.corrcoef(a,b)[:cutoff,cutoff:]
            # corr = jax_corrcoef(a_jaxified, b_jaxified)[:cutoff,cutoff:]
...
df = pd.DataFrame(df)
df.to_csv("filtered_correlations_25k_features_100k_tokens_0.001_thresh_jax.csv")
# %%
# assert False
